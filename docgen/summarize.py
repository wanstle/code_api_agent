...
def _lens_block(lens) -> str:
    ...
    block += (
        ...
        "**粒度约束**:这里是 Module Guide,不是 Detailed API。可以点名关键入口和协作模块,"
        "但不要展开签名、参数、返回值、异常、字段或逐方法说明;拿不准的一律标 `(unknown)`,不要猜。\n"
    )
    return block
...
def generate(index_name: str, progress=print, use_lens: bool = False, only=None,
    ...
    for mod, files in modules:
        ...
        task = (
            ...
            f"Your task is to write a Module Guide: more concrete than the repository Architecture page, "
            f"but still more high-level than Detailed API.\n"
            ...
            f"- **SECTION HEADINGS (## / ###) MUST be in Chinese**: 模块概述, 关键入口, 工作方式, 阅读建议.\n"
            f"  Do NOT write Detailed API sections like 类参考, 函数参考, 字段, 构造方法, 方法, Parameters, Returns, Raises.\n\n"
            f"{ctx}\n\n"
            ...
            f"3. NO Mermaid diagrams, flowcharts, or graphviz. Use prose, compact tables, and plain-text lists.\n"
            f"4. Every table row MUST have the same number of columns as its header. Count after writing each table.\n"
            f"5. Use `file:line` format for source locations.\n"
            f"6. Avoid API-reference detail: no signatures, no parameter tables, no return-value tables, no exception lists, no decorators, no field inventory.\n\n"
            f"### Module Structure Requirements\n"
            f"- Write `## 模块概述` as 3 bullets: 角色定位, 职责边界, 协作关系.\n"
            f"- Write `## 关键入口` as a compact table (入口 | 类型 | 适用场景 | 位置), listing only representative public classes/functions.\n"
            f"- Write `## 工作方式` as 2-4 bullets explaining the module's main runtime/data/control flow at module level.\n"
            f"- Write `## 阅读建议` as 1-3 bullets telling readers which Detailed API/source files to open next for deeper inspection.\n"
            f"- Do NOT include signatures, parameter tables, return-value tables, constructors, methods, fields, examples, or per-method details.\n"
            f"- Architecture page explains the whole repository; this page explains this module's role, boundaries, important entry points, and local flow.\n"
            f"- Detailed API pages cover exact signatures and symbol-level behavior; do not duplicate them here.\n"
            f"- `## 模块概述` must clearly answer:\n"
            ...
            f"- For related symbols, mention important upstream/downstream modules in prose, not as API detail.\n"
            f"- Keep the whole module guide concise: usually 350-700 Chinese characters plus compact tables/lists.\n"
        )
