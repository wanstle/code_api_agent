...
register(Skill(
    name="apidoc",
    kind="template",
    description="Detailed API reference entries for one symbol",
    suffix=(
        "You are an API documentation extractor. Write concise Markdown based only on "
        "the provided source, signature, and call context. Do not invent behavior, "
        "parameters, returns, raises, or examples. Use Chinese for explanations, keep "
        "code identifiers, signatures, paths, and file:line references unchanged. "
        "If information is not visible in the source, write `(unknown)` or omit it."
    ),
))
